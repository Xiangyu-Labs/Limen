...
export function buildDashboardViewModel(
  ...
  return {
    heading: q ? `Results for "${q}"` : 'Timeline',
    emptyMessage: 'No entries found. Start capturing your thoughts.',
    summaryStats: [
      { label: 'Processing', value: counts.pending, tone: 'warning' },
      { label: 'Needs review', value: counts.failed, tone: 'danger' },
      { label: 'Ready', value: counts.done, tone: 'success' },
    ],
    entries: allEntries.map((entry) => ({
      ...entry,
      tags: entry.tags ? JSON.parse(entry.tags) as string[] : [],
      displayTitle: entry.title || 'Untitled Entry',
        ...
        entry.aiStatus === 'pending'
          ...
          : entry.aiStatus === 'failed'
            ...
            : entry.aiStatus === 'done'
              ? 'success'
              : 'muted',
      metaLine: [`${entry.tags ? (JSON.parse(entry.tags) as string[]).length : 0} tags`],
    })),
  };
}
...
export default async function DashboardPage({
  ...
  const viewModel = buildDashboardViewModel(allEntries, q);
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div className="flex flex-col gap-3 rounded-[var(--radius-xl)] border border-border/70 bg-surface px-5 py-5 shadow-sm md:flex-row md:items-end md:justify-between md:px-6">
        <div className="space-y-1">
          <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-primary">Timeline</p>
          <h1 className="text-2xl font-bold tracking-tight text-text">
            {viewModel.heading}
          </h1>
          <p className="text-sm text-muted">Scan recent captures, spot AI status, and jump back into writing faster.</p>
        </div>
      ...
      {viewModel.entries.length === 0 ? (
        <div className="rounded-[var(--radius-xl)] border border-border bg-surface py-20 flex flex-col items-center justify-center text-center shadow-sm">
          ...
          <Link href="/entries/new" className="mt-6 text-primary text-sm font-bold uppercase tracking-widest hover:underline active:scale-[0.99] transition-transform">
