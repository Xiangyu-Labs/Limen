...
test('proxy redirects every unauthenticated private page including unknown paths', () => {
  ...
  assert.deepEqual(decide('/private.json'), {
    type: 'redirect',
    location: '/login',
  });
});
...
test('dashboard API requires a browser session', () => {
  assert.deepEqual(decide('/api/dashboard/entries'), {
    type: 'json',
    status: 401,
    body: { error: 'Unauthorized' },
  });
  assert.deepEqual(decide('/api/dashboard/entries', { hasSession: true }), {
    type: 'next',
  });
});

test('external API requires the independently hashed Bearer token', () => {
  assert.deepEqual(decide('/api/entries'), {
    type: 'json',
    status: 401,
    body: { error: 'Unauthorized' },
  });
  assert.deepEqual(
    decide('/api/entries', { authHeader: 'Bearer api-secret' }),
    { type: 'next' },
  );
  assert.deepEqual(decide('/api/entries', { authHeader: 'Bearer wrong' }), {
    type: 'json',
    status: 401,
    body: { error: 'Unauthorized' },
  });
