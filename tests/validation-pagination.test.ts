...
test('entry validation enforces content, date, and bulk limits', () => {
  assert.equal(
    parseEntryContent('x'.repeat(ENTRY_CONTENT_MAX_LENGTH)).length,
    ENTRY_CONTENT_MAX_LENGTH,
  );
  assert.throws(
    () => parseEntryContent('x'.repeat(ENTRY_CONTENT_MAX_LENGTH + 1)),
    InputValidationError,
  );
  assert.equal(
    parseEntryDate('2024-02-29').toISOString(),
...
test('stored tags tolerate malformed and oversized values', () => {
  ...
  assert.deepEqual(
    parseStoredTags(JSON.stringify([' one ', 'one', 3, 'two'])),
    ['one', 'two'],
  );
});
