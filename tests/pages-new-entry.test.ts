...
test("new entry submit is disabled when loading", async () => {
  const { isEntrySubmitDisabled } = await import("@/app/[locale]/(dashboard)/entries/new/page");
...
test("new entry page derives a datetime-local default timestamp", async () => {
  const { getDefaultCreatedAtValue } = await import("@/app/[locale]/(dashboard)/entries/new/page");
  assert.equal(
